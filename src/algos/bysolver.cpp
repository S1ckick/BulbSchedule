...
void algos::bysolver (Satellites &sats, Observatories &obs) {
    ...
    for (auto &inter: plan)
    {
        ...
        //std::cout << (DURATION(START_MODELLING, inter->start) * 1000)  << "\n";
        for (auto &info : infos)
        {
            if (info->state == State::RECORDING) 
            {
                ...
                vars.insert({id, v});
                id_to_info[id] = info;
                underflow_conditions[info->sat_name] -= v * (int)(1000 * inter->duration * sats.at(info->sat_name).recording_speed);
            ...
            else if (info->state == State::BROADCAST)
            {
                ...
                vars.insert({id, v}); 
                id_to_info[id] = info;
                underflow_conditions[info->sat_name] += v * (int)(1000 * inter->duration * sats.at(info->sat_name).broadcasting_speed);
                uniqueness_conditions_obs[info->obs_name] += v;
                uniqueness_conditions_sat[info->sat_name] += v;
                //printf(",%d=%f*%f", (int)(1000 * inter->duration * sats.at(info->sat_name).broadcasting_speed), inter->duration,  sats.at(info->sat_name).broadcasting_speed);
                optimized += v * (int)(1000 * inter->duration * sats.at(info->sat_name).broadcasting_speed);
            }
            //std::cout << info->sat_name << " " << info->obs_name << " " << int(info->state) << "\n";
        }
